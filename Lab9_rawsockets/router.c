...
int main (int argc, char** argv) {
    ...
    //	}
	//}
    if (pthread_create(&(threads[0]), NULL, (void*(*)(void *))sniffer, (void *)(&sniff_args[0]) )) {
        	fprintf(stderr, "ERROR creating thread %d\n", i);
    	}
...
void sniffer(void* param) {
	...
	sockopt(sockrecv, SOL_SOCKET, SO_BINDTODEVICE, "eth2",4);
	struct sockaddr pctrecvaddr;
    int pktrecvaddr_size = sizeof(pktrecvaddr);
	while(1){
    	if(ret=recvfrom(sockrecvret, packet, 70, 0, &retransaddr, (socklen_t*)&pktrecvaddr_size)==-1){
    	printf("Could not recieve packets from sender node");
    	exit(1);
	       }
	fclose(data->logfile);
...
void process_packet(u_char *args, const struct pcap_pkthdr *header, const u_char *packet) {
	...
	//int handle_idx;
    int sockfrwd;
    sockfrwd=socket(AF_PACKET, SOCK_RAW, htons(ETH_P_ALL));
    sockopt(sockfrwd, SOL_SOCKET, SO_BINDTODEVICE, "eth3",4);
	struct sockaddr_ll dev3={0};
    memset(&dev,0,sizeof(dev3));
	if((dev2.sll_ifindex=if_nametoindex("eth3"))==0)
	   {
	   	perror("if_nameto index failed to obtain");
	   	exit(0);
	   }
	 dev3.sll_family = AF_PACKET;
	 dev3.sll_addr[0] = 0x11;
	 dev3.sll_addr[1] = 0x11;
	 dev3.sll_addr[2] = 0x11;
	 dev3.sll_addr[3] = 0x11;
	 dev3.sll_addr[4] = 0x11;
	 dev3.sll_addr[5] = 0x11;
	 dev3.sll_halen=ETH_ALEN;
	 dev3.sll_protocol=htons(0x1234);
	...
	switch(ret) {
		case P_FORWARD:
			...
			//	exit(1);
			//}
			if(ret=sendto(sockfrwd,packetOut,pktlen,0,(structsockaddr*)&dev3,sizeof(dev3))==-1){
