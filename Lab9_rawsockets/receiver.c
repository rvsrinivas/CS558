...
void send_end( pcap_t *handle_sniffed_nack){
    ...
    u_char packet[PACKET_BUF_SIZE];
    int seqNum = -1;
    struct sockaddr_ll dev5={0};
    memset(&dev,0,sizeof(dev5));
	if((dev5.sll_ifindex=if_nametoindex("eth0"))==0)
	   {
	   	perror("if_nameto index failed to obtain");
	   	exit(0);
	   }
	 dev5.sll_family = AF_PACKET;
	 dev5.sll_addr[0] = 0x11;
	 dev5.sll_addr[1] = 0x11;
	 dev5.sll_addr[2] = 0x11;
	 dev5.sll_addr[3] = 0x11;
	 dev5.sll_addr[4] = 0x11;
	 dev5.sll_addr[5] = 0x11;
	 dev5.sll_halen=ETH_ALEN;
	 dev5.sll_protocol=htons(0x1234);
    int socksendret;
    socksendret=socket(AF_PACKET, SOCK_RAW, htons(ETH_P_ALL));
    n = generate_route_on_resend_packet(packet, 70, ROUTE_ON_RELIABLE, seqNum);
   // print_dummy_packet(packet, n);
    //fprintf(stdout, "Send End atleaast 10 times to %d of size %d\n", dest_addr, n);
    for (i = 0; i < 30; i++){
    	if(ret=sendto(socksendret, packet, 70, (structsockaddr*)&dev5, sizeof(dev5))==-1){
    		printf("Could not send resend packets");
		}
        //if ((ret = pcap_inject(handle_sniffed_nack, packet, n)) < 0){
        //    fprintf(stderr, "Fail to inject packet\n");
		// exit(1);
...
int main(int argc, char *argv[])
{
    ...
    struct sockaddr_ll dev4={0};
    memset(&dev,0,sizeof(dev4));
	if((dev2.sll_ifindex=if_nametoindex("eth0"))==0)
	   {
	   	perror("if_nameto index failed to obtain");
	   	exit(0);
	   }
	 dev4.sll_family = AF_PACKET;
	 dev4.sll_addr[0] = 0x11;
	 dev4.sll_addr[1] = 0x11;
	 dev4.sll_addr[2] = 0x11;
	 dev4.sll_addr[3] = 0x11;
	 dev4.sll_addr[4] = 0x11;
	 dev4.sll_addr[5] = 0x11;
	 dev4.sll_halen=ETH_ALEN;
	 dev4.sll_protocol=htons(0x1234);
    int sockreceive;
    sockreceive=socket(AF_PACKET, SOCK_RAW, htons(ETH_P_ALL));
    struct sockaddr receiveaddr;
    int receiveaddr_size = sizeof(receivesaddr);
    while(1){
    	if(ret=recvfrom(sockreceive, packet, PACKET_BUF_SIZE, 0, &receiveaddr, (socklen_t*)&receiveaddr_size)==-1){
    	printf("Could not recieve receive packets");
    	exit(1);
	       }
    process_packet(packet,sockreceive,dev4);
} 
