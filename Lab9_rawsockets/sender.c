...
void process_packet(const u_char *packet_n,int sockrecvret,struct sockaddr_ll dev2) {
    struct rthdr *rth = (struct rthdr*) packet_n;
    u_char *packetIn = (u_char *) packet_n;
	char err[128];
	struct rthdr *rth1 = (struct rthdr *)packet_n;
	unsigned int size =  rth1->size;
        //int size = (int) header->len;
	int ret = 0, retr=0, hdrlen, payload_size;
	...
	switch(ret) {
        case P_APPRESPONSE:
                printf("Entered Switch case APPRESPONSE\n");
               ...
               // print_dummy_packet(packetOut, pktlen);
                ...
                printf("Retrans Packet Size is %d\n",pktlen);
                retr=sendto(sockrecvret,packetOut,pktlen,0,(struct sockaddr*)&dev2,sizeof(dev2));
                if(retr==-1){
                printf("Cannot send packet using sendto of retransmission");
                exit(1);
}
        default:
            break;
...
void* resend_packet(void* a)
{
    ...
    u_char packet[70];
    int ret=0;
    struct sockaddr_ll dev2={0};
    memset(&dev2,0,sizeof(dev2));
	if((dev2.sll_ifindex=if_nametoindex("eth3"))==0)
	   {
	   	perror("if_nameto index failed to obtain");
	   	exit(0);
	   }
	 dev2.sll_family = AF_PACKET;
	 dev2.sll_addr[0] = 0x11;
	 dev2.sll_addr[1] = 0x11;
	 dev2.sll_addr[2] = 0x11;
	 dev2.sll_addr[3] = 0x11;
	 dev2.sll_addr[4] = 0x11;
	 dev2.sll_addr[5] = 0x11;
	 dev2.sll_halen=ETH_ALEN;
	 dev2.sll_protocol=htons(0x1234);
    int sockrecvret;
    sockrecvret=socket(AF_PACKET, SOCK_RAW, htons(ETH_P_ALL));
    struct sockaddr retransaddr;
    int retransaddr_size = sizeof(retransaddr);
    while(1){
    	ret=recvfrom(sockrecvret, packet, 70, 0, &retransaddr, (socklen_t*)&retransaddr_size);
        if(ret==-1){
    	printf("Could not recieve retransmission packets");
    	exit(1);
	       }
    //print_data(stdout,packet,70);
    //printf("Received NACK/END Packet\n");
    process_packet(packet,sockrecvret,dev2);
}
}
...
int main(int argc, char *argv[])
{
	...
	setsockopt(sockrecv, SOL_SOCKET, SO_BINDTODEVICE, "eth3",4);
...
//    filesize = ftell(fp_read);
	...
	if((dev.sll_ifindex=if_nametoindex("eth3"))==0)
	   {
	   	perror("if_nameto index failed to obtain");
	   	exit(0);
	   }
    ...
    while (seqNum < no_of_packets) {
        ...
        //if ((ret = pcap_inject(handle_sniffed, packet, pktlen)) < 0){
          ...
          if(ret=sendto(sockfd,packet,pktlen,0,(struct sockaddr*)&dev,sizeof(dev))==-1){
          printf("Cannot send packet using sendto\n");
          exit(1);
      }
		// exit(1);
        seqNum++;
